...
// ─── UNIVERSE: SGD-only, ≥200M mkt cap, 207 stocks, 16 sectors (27 Jun 2026) ─
...
const TOP_SECTORS = 3;
const TRADE_NAMES = 2;   // names traded per qualifying sector
const SCORE_NAMES = 5;   // FIX #4: expanded from 3 to 5 for sector scoring
...
async function fetchPriceHistory(ticker) {
  ...
  return timestamps.map((t,i) => ({t, c:closes[i]})).filter(x => x.c != null);
}
...
function computeReturns(history) {
  ...
  return { w1:getR(TRADING_DAYS.w1), m1:getR(TRADING_DAYS.m1), m3:getR(TRADING_DAYS.m3), m6:getR(TRADING_DAYS.m6), latest:last };
}

// FIX #4: compute score from all available periods (never show — for missing data)
...
function groupBySector(universe) {
  const map = {};
  universe.forEach(s => { if (!map[s.sector]) map[s.sector]=[]; map[s.sector].push(s); });
...
const rankColor = r => r===1?"#f59e0b":r===2?"#94a3b8":"#b45309";
...
export default function SGXMomentum() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    setStatus("loading"); setErrors([]); setStockData({}); setSectorResults([]); setSignals([]);
    const sectorMap = groupBySector(STOCK_UNIVERSE);

    // Collect top SCORE_NAMES tickers per sector (FIX #4: now 5)
    const tickersNeeded = new Set();
    Object.values(sectorMap).forEach(stocks => stocks.slice(0,SCORE_NAMES).forEach(s => tickersNeeded.add(s.ticker)));
    ...
    for (let i=0; i<tickerList.length; i++) {
      ...
      try {
        ...
        results[ticker] = {returns:computeReturns(history), error:null};
      } catch(e) {
    ...
    Object.entries(sectorMap).forEach(([sector,stocks]) => {
      // FIX #4: score from top 5, trade top 2
      const top5 = stocks.slice(0,SCORE_NAMES);
      const top2 = stocks.slice(0,TRADE_NAMES);

      const stockScores = top5.map(s => {
      ...
      const valid = stockScores.filter(s => s.score!=null);
      const sectorScore = valid.length>0 ? valid.reduce((a,b)=>a+b.score,0)/valid.length : null;
      const valid3m = stockScores.filter(s => s.returns?.m3!=null);
      const sector3m = valid3m.length>0 ? valid3m.reduce((a,b)=>a+b.returns.m3,0)/valid3m.length : null;

      sectors.push({
        sector, sectorScore, sector3m, stockScores, tradeStocks:top2,
        dataQuality:`${valid.length}/${top5.length} stocks with data`
      });
    });
    ...
    if (top3.length===0) {
      newSignals.push({type:"CASH", message:"All sectors ≤0% — go 100% cash."});
    } else {
      top3.forEach((s,rank) => {
        const wasIn = prevTop3?.some(p=>p.sector===s.sector);
        s.tradeStocks.forEach(stock => {
          newSignals.push({type:wasIn?"HOLD":"BUY", sector:s.sector, rank:rank+1, sectorScore:s.sectorScore, stock});
        });
      });
      if (prevTop3) {
        prevTop3.forEach(prev => {
          if (!top3.some(s=>s.sector===prev.sector))
            newSignals.push({type:"SELL", sector:prev.sector, message:`${prev.sector} dropped out — sell both positions.`});
        });
      }
    }
  ...
  const S = { // style shortcuts
    page:{background:"#0f172a",minHeight:"100vh",color:"#e2e8f0",fontFamily:"'Inter',system-ui,sans-serif"},
    header:{background:"linear-gradient(135deg,#1e293b,#0f172a)",borderBottom:"1px solid #1e3a5f",padding:"20px 24px 16px"},
    card:{background:"#1e293b",border:"1px solid #334155",borderRadius:10,overflow:"hidden",marginBottom:14},
    innerCard:{background:"#0f172a",borderRadius:8,padding:"12px 14px"},
    pill:(t)=>({background:signalColor(t)+"22",color:signalColor(t),padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700,border:`1px solid ${signalColor(t)}44`}),
    th:{padding:"8px 12px",textAlign:"left",color:"#64748b",fontWeight:600,fontSize:11,letterSpacing:"0.05em",textTransform:"uppercase"},
    td:(color)=>({padding:"9px 12px",color:color||"#e2e8f0"}),
  };
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
          ...
          <div style={{display:"flex",gap:10}}>
            {status==="loading" && <button onClick={()=>{abortRef.current=true;setStatus("idle");}} style={{padding:"8px 16px",background:"#1e293b",border:"1px solid #334155",borderRadius:8,color:"#94a3b8",cursor:"pointer",fontSize:13}}>Stop</button>}
            <button onClick={fetchAll} disabled={status==="loading"} style={{padding:"10px 22px",background:status==="loading"?"#1e3a5f":"linear-gradient(135deg,#2563eb,#1d4ed8)",border:"none",borderRadius:8,color:"#fff",cursor:status==="loading"?"not-allowed":"pointer",fontSize:14,fontWeight:600,boxShadow:"0 2px 8px rgba(37,99,235,0.3)"}}>
        ...
        {status==="loading" && (
          <div style={{marginTop:12}}>
            <div style={{background:"#1e293b",borderRadius:4,height:4,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${pct}%`,background:"linear-gradient(90deg,#2563eb,#7c3aed)",transition:"width 0.3s",borderRadius:4}}/>
            </div>
      ...
      {status==="done" && allNegative && (
        <div style={{background:"linear-gradient(135deg,#450a0a,#7f1d1d)",margin:"16px 24px 0",borderRadius:10,padding:"14px 20px",border:"1px solid #991b1b",display:"flex",alignItems:"center",gap:12}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"#fca5a5",opacity:0.7,marginTop:2}}>No sectors qualify (score &gt;0%). Hold cash until market improves.</div>
          </div>
        </div>
      )}
      ...
      <div style={{padding:"16px 24px 0",borderBottom:"1px solid #1e293b"}}>
        <div style={{display:"flex",gap:4}}>
          {[["signals","🎯 Trade Signals"],["ranking","📊 Sector Ranking"],["detail","🔍 Score Detail"],["errors",`⚠ Errors (${errors.length})`]].map(([id,label])=>(
            <button key={id} onClick={()=>setActiveTab(id)} style={{padding:"8px 16px",background:activeTab===id?"#1e3a5f":"transparent",border:activeTab===id?"1px solid #2563eb":"1px solid transparent",borderRadius:"8px 8px 0 0",color:activeTab===id?"#60a5fa":"#475569",cursor:"pointer",fontSize:13,fontWeight:activeTab===id?600:400}}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{padding:"20px 24px"}}>
          ...
          <div>
            {status==="idle" && (
              <div style={{textAlign:"center",padding:"60px 20px",color:"#334155"}}>
                ...
                <div style={{fontSize:13,color:"#334155",marginTop:8}}>207 SGX stocks · SGD ≥200M · 16 sector groups · Top-5 scoring · Top-2 trading</div>
              </div>
            )}
            {status==="done" && (
              <div>
                ...
                {top3.map((s,idx)=>{
                  const wasIn = prevTop3?.some(p=>p.sector===s.sector);
                  return (
                    <div key={s.sector} style={{...S.card,border:`1px solid ${idx===0?"#854d0e":"#334155"}`}}>
                      <div style={{padding:"14px 18px",background:idx===0?"linear-gradient(90deg,rgba(245,158,11,0.08),transparent)":"transparent",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid #0f172a"}}>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          ...
                          <div>
                            ...
                            <div style={{fontSize:11,color:"#64748b"}}>Scored: {s.dataQuality} · Traded: top 2</div>
                          </div>
                        </div>
                      ...
                      {/* FIX #4 — show score breakdown for ALL 5 scoring stocks */}
                      <div style={{padding:"10px 18px",borderBottom:"1px solid #0f172a"}}>
                        <div style={{fontSize:10,color:"#475569",fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:6}}>Sector Score Calculation (Top {Math.min(SCORE_NAMES,s.stockScores.length)} by Mkt Cap)</div>
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                            <thead>
                              <tr>
                                {["Role","Stock","Code","Mkt Cap","1W","1M","3M","6M","Score"].map(h=>(
                                  <th key={h} style={{...S.th,padding:"4px 8px"}}>{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {s.stockScores.map((st,i)=>(
                                <tr key={st.code} style={{borderTop:"1px solid #0f172a",background:i<TRADE_NAMES?"rgba(37,99,235,0.05)":"transparent"}}>
                                  <td style={{padding:"5px 8px"}}>
                                    <span style={{fontSize:10,fontWeight:700,color:i<TRADE_NAMES?"#10b981":"#64748b"}}>{i<TRADE_NAMES?"TRADE":"SCORE"}</span>
                                  </td>
                                  <td style={{padding:"5px 8px",color:"#e2e8f0",fontWeight:i<TRADE_NAMES?600:400}}>{st.name}</td>
                                  <td style={{padding:"5px 8px",color:"#64748b",fontFamily:"monospace"}}>{st.code}</td>
                                  <td style={{padding:"5px 8px",color:"#94a3b8"}}>SGD {fmtCap(st.mktCapM)}</td>
                                  <td style={{padding:"5px 8px",color:pctColor(st.returns?.w1)}}>{fmtPct(st.returns?.w1,1)}</td>
                                  <td style={{padding:"5px 8px",color:pctColor(st.returns?.m1)}}>{fmtPct(st.returns?.m1,1)}</td>
                                  <td style={{padding:"5px 8px",color:pctColor(st.returns?.m3),fontWeight:600}}>{fmtPct(st.returns?.m3,1)}</td>
                                  <td style={{padding:"5px 8px",color:pctColor(st.returns?.m6)}}>{fmtPct(st.returns?.m6,1)}</td>
                                  <td style={{padding:"5px 8px",color:pctColor(st.score),fontWeight:700}}>{fmtPct(st.score)}</td>
                                </tr>
                              ))}
                              <tr style={{borderTop:"2px solid #334155",background:"rgba(37,99,235,0.08)"}}>
                                <td colSpan={8} style={{padding:"5px 8px",color:"#94a3b8",fontSize:11,fontWeight:600}}>SECTOR SCORE (avg of {s.stockScores.filter(x=>x.score!=null).length} valid scores)</td>
                                <td style={{padding:"5px 8px",color:pctColor(s.sectorScore),fontWeight:700,fontSize:14}}>{fmtPct(s.sectorScore)}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                              ...
                              {d?.returns && (
                                <>
                                  ...
                                  <div style={{fontSize:11,color:"#64748b"}}>
                                    Momentum score: <span style={{color:pctColor(weightedScore(d.returns)),fontWeight:700}}>{fmtPct(weightedScore(d.returns))}</span>
                                  </div>
                                </>
                              )}
                              {d?.error && <div style={{fontSize:11,color:"#ef4444",marginTop:4}}>⚠ {d.error}</div>}
                              {!d?.returns && !d?.error && <div style={{fontSize:11,color:"#475569",marginTop:4}}>No price data</div>}
                            </div>
                          );
                        })}
                      </div>
                ...
                {signals.filter(s=>s.type==="SELL").map((s,i)=>(
                  <div key={i} style={{background:"#1e293b",border:"1px solid #7f1d1d",borderRadius:10,padding:"14px 18px",marginBottom:10,display:"flex",alignItems:"center",gap:12}}>
                    <span style={S.pill("SELL")}>SELL</span>
        ...
        {activeTab==="ranking" && (
          <div>
            ...
            {sectorResults.length>0 && (
              <>
                <div style={{marginBottom:12,fontSize:13,color:"#475569"}}>Score = 1W×10% + 1M×35% + 3M×35% + 6M×15% averaged across top 5 names by mkt cap. Tiebreaker within 0.5%: 3M return. ✦ = top 3 qualifying sectors.</div>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead>
                      <tr style={{borderBottom:"1px solid #334155"}}>
                        {["Rank","Sector","Score","1W avg","1M avg","3M avg","6M avg","Qualifies","Data"].map(h=>(
                    ...
                    <tbody>
                      {sectorResults.map((s,i)=>{
                        ...
                        return (
                          <tr key={s.sector} style={{borderBottom:"1px solid #1e293b",background:inTop3?"rgba(37,99,235,0.07)":"transparent"}}>
                            <td style={S.td(inTop3?rankColor(i+1):"#475569")}><strong>#{i+1}</strong></td>
                            <td style={S.td(inTop3?"#f1f5f9":"#94a3b8")}>{inTop3&&<span style={{marginRight:6}}>✦</span>}{s.sector}</td>
                            <td style={S.td(pctColor(s.sectorScore))}><strong>{fmtPct(s.sectorScore)}</strong></td>
                            <td style={S.td(pctColor(avg("w1")))}>{fmtPct(avg("w1"),1)}</td>
                            <td style={S.td(pctColor(avg("m1")))}>{fmtPct(avg("m1"),1)}</td>
                            <td style={{...S.td(pctColor(avg("m3"))),fontWeight:600}}>{fmtPct(avg("m3"),1)}</td>
                            <td style={S.td(pctColor(avg("m6")))}>{fmtPct(avg("m6"),1)}</td>
                            <td style={S.td()}>
                            ...
                            <td style={S.td("#475569")}>{s.dataQuality}</td>
                          </tr>
                        );
                      })}
                    </tbody>
        ...
        {activeTab==="detail" && (
          <div>
            {sectorResults.length===0 && <div style={{color:"#475569",textAlign:"center",padding:40}}>Run refresh to see detail.</div>}
            <div style={{marginBottom:12,fontSize:13,color:"#475569"}}>Shows top 5 stocks used for sector scoring. TRADE = also traded (top 2). SCORE = scoring only.</div>
            {sectorResults.map(s=>(
              <div key={s.sector} style={{marginBottom:12}}>
                <div onClick={()=>setExpandedSector(expandedSector===s.sector?null:s.sector)}
                  ...
                  <div style={{fontWeight:600,color:"#f1f5f9"}}>{s.sector} <span style={{fontSize:11,color:"#475569",fontWeight:400}}>({s.dataQuality})</span></div>
                  <div style={{display:"flex",gap:16,alignItems:"center"}}>
                    <span style={{color:pctColor(s.sectorScore),fontWeight:700}}>{fmtPct(s.sectorScore)}</span>
                    <span style={{color:"#475569",fontSize:12}}>{expandedSector===s.sector?"▲":"▼"}</span>
                  </div>
                </div>
                {expandedSector===s.sector && (
                  <div style={{background:"#0f172a",border:"1px solid #1e293b",borderTop:"none",borderRadius:"0 0 8px 8px",overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead>
                        <tr style={{borderBottom:"1px solid #1e293b"}}>
                          {["Role","Stock","Code","Mkt Cap","1W","1M","3M","6M","Score"].map(h=>(
                            <th key={h} style={{...S.th,padding:"7px 12px"}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {s.stockScores.map((st,i)=>(
                          <tr key={st.code} style={{borderBottom:"1px solid #1e293b",background:i<TRADE_NAMES?"rgba(37,99,235,0.04)":"transparent"}}>
                            <td style={{padding:"7px 12px"}}>
                              <span style={{fontSize:10,color:i<TRADE_NAMES?"#10b981":"#64748b",fontWeight:700}}>{i<TRADE_NAMES?"TRADE":"SCORE"}</span>
                            </td>
                            <td style={{padding:"7px 12px",color:"#e2e8f0",fontWeight:i<TRADE_NAMES?600:400}}>{st.name}</td>
                            <td style={{padding:"7px 12px",color:"#64748b",fontFamily:"monospace"}}>{st.code}</td>
                            <td style={{padding:"7px 12px",color:"#94a3b8"}}>SGD {fmtCap(st.mktCapM)}</td>
                            <td style={{padding:"7px 12px",color:pctColor(st.returns?.w1)}}>{fmtPct(st.returns?.w1,1)}</td>
                            <td style={{padding:"7px 12px",color:pctColor(st.returns?.m1)}}>{fmtPct(st.returns?.m1,1)}</td>
                            <td style={{padding:"7px 12px",color:pctColor(st.returns?.m3),fontWeight:600}}>{fmtPct(st.returns?.m3,1)}</td>
                            <td style={{padding:"7px 12px",color:pctColor(st.returns?.m6)}}>{fmtPct(st.returns?.m6,1)}</td>
                            <td style={{padding:"7px 12px",color:pctColor(st.score),fontWeight:700}}>{fmtPct(st.score)}</td>
                          </tr>
                        ))}
                        <tr style={{borderTop:"2px solid #334155",background:"rgba(37,99,235,0.06)"}}>
                          <td colSpan={8} style={{padding:"7px 12px",color:"#94a3b8",fontSize:11,fontWeight:600}}>
                            SECTOR SCORE — weighted avg of {s.stockScores.filter(x=>x.score!=null).length} valid scores · formula: 1W×10%+1M×35%+3M×35%+6M×15%
                          </td>
                          <td style={{padding:"7px 12px",color:pctColor(s.sectorScore),fontWeight:700,fontSize:14}}>{fmtPct(s.sectorScore)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
        ...
        {activeTab==="errors" && (
          <div>
            {errors.length===0 && status==="done" && <div style={{color:"#10b981",textAlign:"center",padding:40}}>✓ No errors — all tickers fetched successfully.</div>}
            {errors.length===0 && status!=="done" && <div style={{color:"#475569",textAlign:"center",padding:40}}>Run refresh to see error log.</div>}
            {errors.length>0 && (
              <>
                <div style={{marginBottom:12,color:"#f59e0b",fontSize:13}}>⚠ {errors.length} tickers failed. These stocks are excluded from sector scoring.</div>
                <div style={{background:"#1e293b",borderRadius:8,overflow:"hidden"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                    <thead><tr style={{borderBottom:"1px solid #334155"}}><th style={S.th}>Ticker</th><th style={S.th}>Error</th></tr></thead>
                    <tbody>
        ...
        <div style={{marginTop:32,borderTop:"1px solid #1e293b",paddingTop:16,display:"flex",flexWrap:"wrap",gap:20,fontSize:11,color:"#334155"}}>
          ...
          <div><span style={{color:"#475569",fontWeight:600}}>Sector score:</span> avg of top 5 by mkt cap</div>
          <div><span style={{color:"#475569",fontWeight:600}}>Traded:</span> top 2 per qualifying sector</div>
          <div><span style={{color:"#475569",fontWeight:600}}>Tiebreaker:</span> 3M return when gap ≤0.5%</div>
          <div><span style={{color:"#475569",fontWeight:600}}>Positions:</span> Top 3 sectors × 2 names = max 6</div>
